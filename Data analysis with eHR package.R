...
ehr_path <- dirname(system.file("ehr_data", "ehr_Clinical.txt",
       package = "rEHR"))
...
head(diabetes_codes)
 select_events(db,tab="Clinical", columns = c("patid", "eventdate",
               "medcode"), 
...
sqldf("SELECT patid, practid, gender, yob, deathdate from Patient WHERE deathdate IS NOT NULL LIMIT 6",
      connection=db)
...
#wrap_sql_query(-"SELECT * FROM clinical WHERE practid == # 1 AND medcodes in # 2",practice,medcodes1)
